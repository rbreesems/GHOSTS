...
{
    public class BrowserEdge : BaseBrowserHandler
    {
        public BrowserEdge(TimelineHandler handler)
        {
            base.Init(handler);
            BrowserType = HandlerType.BrowserEdge;
            StartEx(handler);
        ...
        public virtual void StartEx(TimelineHandler handler)
        {
            ...
            {
                hasRunSuccessfully = MSEdgeEx(handler);
            }
        }
        private static string GetInstallLocation()
        {
            var path = @"C:\Program Files\Microsoft\Edge\Application\msedge.exe";
            if (File.Exists(path))
            {
                return path;
            }
        ...
        public bool MSEdgeEx(TimelineHandler handler)
        {
            try
            {
                using (Driver = GetDriver(handler))
                {
                    ...
                    if (handler.Loop)
                    {
                        while (true)
                        {
                            ...
                            {
                                throw new Exception("MSEdge window handle not available");
                            }
            ...
            finally
            {
                ...
                try
                {
                    ProcessManager.KillProcessAndChildrenByName(ProcessManager.ProcessNames.MSEdgeDriver);
                    ProcessManager.KillProcessAndChildrenByName(ProcessManager.ProcessNames.MSEdge);
                }
                catch { }
        ...
        internal static IWebDriver GetDriver(TimelineHandler handler)
        {
            ...
            if (handler.HandlerArgs != null)
            {
                ...
                if (handler.HandlerArgs.ContainsKey("user-data-dir"))
                {
                    ...
                    Log.Trace($"Loading edgedriver profile from {profile}...");
                }
                ...
                {
                    options.AddLocalStatePreference("profile.managed_default_content_settings.stylesheets", 2);
                }
                ...
                if (handler.HandlerArgs.ContainsKey("ua-string"))
                {
                    switch (handler.HandlerArgs["ua-string"].ToString().ToLower())
                    {
                        case "random":
                            ...
                            break;
                        case "strict":
                            options.AddArgument($"--user-agent={UserAgentManager.GetBrowserSpecific("edge")}");
                            break;
                        default:
